...
// Fixed gas limits to avoid wallet gas estimation failures
const GAS_LIMITS = {
  APPROVE: 80000,
  SWAP: 220000,
  DICE_PLAY: 160000,
  ...
  try {
    ...
    if (activeSwapDirection === "VIN_TO_MON") {
      ...
      if (currentAllowance.lt(vinAmountBN)) {
        ...
        const txApprove = await vinWrite.approve(SWAP_CONTRACT_ADDRESS, approveAmount, { gasLimit: GAS_LIMITS.APPROVE });
        await txApprove.wait();
      }

      setStatus(statusId, "Sending swap transaction (VIN → MON)...");
      const tx = await swapWrite.swapVINtoMON(vinAmountBN, { gasLimit: GAS_LIMITS.SWAP });
      ...
      const tx = await swapWrite.swapMONtoVIN({ value: monAmountBN, gasLimit: GAS_LIMITS.SWAP });
      const receipt = await tx.wait();
      setStatus(
        statusId,
        `Swap completed. Tx: ${shortenAddress(receipt.transactionHash)}`
...
async function diceApprove() {
  ...
  try {
    ...
    const tx = await vinWrite.approve(DICE_CONTRACT_ADDRESS, approveAmountBN, { gasLimit: GAS_LIMITS.APPROVE });
    await tx.wait();
...
async function dicePlay() {
  ...
  try {
    const tx = await diceWrite.play(amountBN, choiceEnum, clientSeed, { gasLimit: GAS_LIMITS.DICE_PLAY });
...
async function lottoApprove() {
  ...
  try {
    ...
    const tx = await vinWrite.approve(LOTTO_CONTRACT_ADDRESS, approveAmountBN, { gasLimit: GAS_LIMITS.APPROVE });
    await tx.wait();
...
async function lottoPlay() {
  ...
  try {
    ...
    const tx = await lottoWrite.play(bets, { gasLimit: GAS_LIMITS.LOTTO_PLAY });
